...
for epoch=start+1:opts.numEpochs
  ...
  if opts.plotStatistics
    ...
    for p = plots
      ...
      plot(1:epoch, values','.-','linewidth', lw) ;
      xlabel('epoch') ;
      title(p) ;
      legend(leg{:}) ;
      grid on ;
...
% % GE: compute regularization term contribution in loss (same within epoch for all minibatches)
% if opts.useReg
%     res(numel(net.layers)+1).reg = vl_nnreg(net, 'regType', opts.regType, 'gpus', opts.gpus);
% end
...
for l=numel(net.layers):-1:1
  for j=1:numel(res(l).dzdw)
    ...
    % accumulate gradients from multiple labs (GPUs) if needed
    if numGpus > 1
